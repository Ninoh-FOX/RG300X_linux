				     ...
				     struct device *dev, void *panel_pdata)
{
	...
	ret = M1700_panel_ops.init(out_panel, dev, panel_pdata);
	if (ret)
		return ret;
...
static void gcw0_panel_enable(void *panel)
{
	...
	__gpio_as_pwm(1);
	M1700_panel_ops.enable(panel);
}

static void gcw0_panel_disable(void *panel)
{
	M1700_panel_ops.disable(panel);
...
static struct M1700_platform_data gcw0_panel_pdata = {
